...
type
  TfrmPDV = class(TForm)
    ...
    btnFinalizarCompra: TSpeedButton;
    btnCancelarCompra: TSpeedButton;
    ...
    procedure btnCancelarCompraClick(Sender: TObject);
...
begin
  try
    ...
    if not btnCancelarCompra.Enabled then
...
begin
  try
    ...
    if (edtCodigoRemover.Text = EmptyStr) then
          begin
            Application.MessageBox(Pchar('Para remover o produto é necessário informar o código'),Pchar('Atenção'),MB_ICONWARNING + MB_OK);
    ...
    begin
      Application.MessageBox(PChar('O produto informado não esta no cadastrado, verifique!'),PChar('Atenção!'),MB_ICONWARNING + MB_OK);
      edtCodigoRemover.SetFocus;
      Exit;
    end;
...
procedure TfrmPDV.btnCancelarCompraClick(Sender: TObject);
begin
  try
...
procedure TfrmPDV.btnFinalizarCompraClick(Sender: TObject);
begin
  FinalizarCompra;
  ResetarTela;
end;

procedure TfrmPDV.btnNovoPedidoClick(Sender: TObject);
begin
  try
...
procedure TfrmPDV.btnTrocarUsuarioClick(Sender: TObject);
begin
  if PedidoAberto then
  begin
    Application.MessageBox(Pchar('Existe um pedido aberto, para realizar esta ação é necessário que ele seja finalizado ou cancelado.'), Pchar('Atenção!'), MB_ICONWARNING + MB_OK);
